...
#endif
static int led_on = 0;
static int gpio12_state = 0;
static int gpio13_state = 0;
static int gpio14_state = 0;
static int gpio15_state = 0;
static int spd_state = 0;
...
static ra_filter_t g_ra_filter;

static ra_filter_t *g_ra_filter_init(ra_filter_t *filter, size_t sample_size)
{
...
static int g_ra_filter_run(ra_filter_t *filter, int value)
{
    if (!filter->values)
    {
        return value;
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    while (true)
    {
        ...
        frame_time /= 1000;
#if ARDUHAL_LOG_LEVEL >= ARDUHAL_LOG_LEVEL_INFO
        uint32_t avg_frame_time = g_ra_filter_run(&g_ra_filter, frame_time);
#endif
...
static esp_err_t duty_handler(httpd_req_t *req)
{
    char *buf = NULL;
    char pin_str[32];
    char val_str[32];
    ...
    ESP_LOGI(TAG, "Set Duty: pin: %d, value: %d", pin, val);
    Serial.printf("Set Duty: pin: %d, value: %d\n", pin, val);

    int channel = -1;
    if (pin == 12) channel = 4;
    else if (pin == 13) channel = 5;
    else if (pin == 14) channel = 6;
    else if (pin == 15) channel = 7;
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    Serial.printf("Received command: var=%s, val=%s (int: %d)\n", variable, value, val);
    int res = 0;

    // First, handle LED commands independently of camera sensor state
    if (!strcmp(variable, "led_intensity")) {
        led_duty = val;
        ESP_LOGI(TAG, "Intensity command received. led_on: %d, led_duty: %d", led_on, led_duty);
        if (led_on) {
            ESP_LOGI(TAG, "Setting duty to %d", led_duty);
            ledc_set_duty(CONFIG_LED_LEDC_SPEED_MODE, CONFIG_LED_LEDC_CHANNEL, led_duty);
            ledc_update_duty(CONFIG_LED_LEDC_SPEED_MODE, CONFIG_LED_LEDC_CHANNEL);
        }
    }
    else if (!strcmp(variable, "led")) {
        led_on = val;
        ESP_LOGI(TAG, "LED state command received. led_on: %d, led_duty: %d", led_on, led_duty);
        if (led_on) {
            ESP_LOGI(TAG, "Turning ON. Setting duty to %d", led_duty);
            ledc_set_duty(CONFIG_LED_LEDC_SPEED_MODE, CONFIG_LED_LEDC_CHANNEL, led_duty);
            ledc_update_duty(CONFIG_LED_LEDC_SPEED_MODE, CONFIG_LED_LEDC_CHANNEL);
        } else {
            ESP_LOGI(TAG, "Turning OFF. Setting duty to 0");
            ledc_set_duty(CONFIG_LED_LEDC_SPEED_MODE, CONFIG_LED_LEDC_CHANNEL, 0);
            ledc_update_duty(CONFIG_LED_LEDC_SPEED_MODE, CONFIG_LED_LEDC_CHANNEL);
        }
    }
    else {
        // Then, handle all other camera-related commands
        ...
        else if (!strcmp(variable, "gpio12")) {
            gpio12_state = val;
            if (val) {
                Serial.println("GPIO12 is now ON");
                // Duty is set by slider, so we don't set it here
            } else {
                Serial.println("GPIO12 is now OFF");
                ledcWrite(4, 0); // Turn off PWM
            }
            res = 0;
        }
        else if (!strcmp(variable, "gpio13")) {
            gpio13_state = val;
            if (val) {
                Serial.println("GPIO13 is now ON");
            } else {
                Serial.println("GPIO13 is now OFF");
                ledcWrite(5, 0);
            }
            res = 0;
        }
        else if (!strcmp(variable, "gpio14")) {
            gpio14_state = val;
            if (val) {
                Serial.println("GPIO14 is now ON");
            } else {
                Serial.println("GPIO14 is now OFF");
                ledcWrite(6, 0);
            }
            res = 0;
        }
        else if (!strcmp(variable, "gpio15")) {
            gpio15_state = val;
            if (val) {
                Serial.println("GPIO15 is now ON");
            } else {
                Serial.println("GPIO15 is now OFF");
                ledcWrite(7, 0);
            }
            res = 0;
        }
        else if (!strcmp(variable, "spd")) {
            spd_state = val;
            res = 0;
        }
#if CONFIG_ESP_FACE_DETECT_ENABLED
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    *p++ = '{';
    ...
    p += sprintf(p, ",\"gpio12\":%u", gpio12_state);
    p += sprintf(p, ",\"gpio13\":%u", gpio13_state);
    p += sprintf(p, ",\"gpio14\":%u", gpio14_state);
    p += sprintf(p, ",\"gpio15\":%u", gpio15_state);
    p += sprintf(p, ",\"spd\":%u", spd_state);
#ifdef CONFIG_LED_ILLUMINATOR_ENABLED
...
void startCameraServer()
{
    ...
    ledc_timer_config_t ledc_timer = {
        .speed_mode = CONFIG_LED_LEDC_SPEED_MODE,
        .duty_resolution = LEDC_TIMER_8_BIT,
        .timer_num = CONFIG_LED_LEDC_TIMER,
        .freq_hz = 5000,
    ...
    ledc_set_duty(CONFIG_LED_LEDC_SPEED_MODE, CONFIG_LED_LEDC_CHANNEL, 0);
    ledc_update_duty(CONFIG_LED_LEDC_SPEED_MODE, CONFIG_LED_LEDC_CHANNEL);
    ESP_LOGI(TAG, "LED Initialized and turned OFF");
#endif
    ...
    httpd_uri_t duty_uri = {
        .uri = "/duty",
        .method = HTTP_GET,
        .handler = duty_handler,
        .user_ctx = NULL
    };

    g_ra_filter_init(&g_ra_filter, 20);
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        httpd_register_uri_handler(camera_httpd, &duty_uri);
    }
